(function ($) {
    ...
    function processArray(template, data, settings) {
        ...
        if (settings.paged) {
            let startNo = (settings.pageNo - 1) * settings.elemPerPage;
    ...
    function prepareTemplateFromCache(template, selection, data, settings) {
        let $templateContainer = templates[template].clone();
    ...
    function prepareTemplate(template, data, settings) {
        ...
        $(this).each(function () {
            let $templateHtml = template.children().clone(true);
    ...
    function valueIsAllowedByBindingOptions(bindingOptionsContainer, value, settings) {
        ...
        let bindingOptions = getBindingOptions(bindingOptionsContainer, settings);
        ...
        } else return !(bindingOptions.ignoreEmptyString && value === "");
    }
    ...
    function applyFormatters($elem, value, attr, settings) {
        let formatterTarget = $elem.attr("data-format-target"),
            formatter;

        if (formatterTarget === attr || (!formatterTarget && attr === "content")) {
            formatter = $elem.attr("data-format");
            if (formatter && typeof formatters[formatter] === "function") {
                let formatOptions = $elem.attr("data-format-options");
                return (function (formatterSettings) {
    ...
    addTemplateFormatter("nestedTemplateFormatter", function (value, options, internalSettings) {
        ...
        let parentElement = options.parentElement || "div";
        let template = options.template || options;
